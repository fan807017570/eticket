...
<html>
	<head>
		...
		<script type="text/javascript">
			...
			function jumpPages(){
				...
				jQuery("#currentPage").val(pageNum);
				jQuery("#listform").submit();
			}
		</script>
		 ...
		 $(document).ready(function(){
		  ...
		  function getColumns(){
			...
			//IE8出现调用两次分页处理   如果存在调用两次，则删掉一次
			/**
			$(document).ready(function(){
				var pageTimes = $(".pagination").length;
				if(pageTimes>2){
	...
	<form id="listform" name="search" method="post" action="${buyer_portal_domain}/view/cpnweb/cpnListByClass">
		<!--  --> 
	<input type="hidden" name="classId" id="classId" value="${classId!}" />   
	<input type="hidden" name="classLevel" id="classLevel" value="${classLevel!}" /> 
