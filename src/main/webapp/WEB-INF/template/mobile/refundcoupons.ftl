...
<@htmlHead title="券管理">
<!-- 	<link rel="stylesheet" href="${buyer_portal_static_resources_domain}/css/index.css" /> -->
...
<#escape x as x?html>
	...
	<div class="myTicket">
		<header class="public_header">
				<ul class="tabs">
					...
					<li><a href="#" class="active">退款单</a></li>
				</ul>
			</header>
		<div class="notFound">
		...
		<div class="list coupon-list-cantainer">
			...
			<div class="item" >
				...
				<div class="bottom">
					<div class="left">有效期：{{coupon.startDate}} - {{coupon.endDate}}</div> 
						<div class="right"> 
							<a href="javascript:window.location.replace('${buyer_weixin_domain}/view/weixin/refunddetail?couponId={{coupon.couponId}}')">退款详情<img src="${buyer_portal_static_resources_domain}/img/icon_double_arrow_right.png"></a>
						</div>
				</div>
			</div>
	{{/each}}
	...
	<script type="text/javascript">
		...
		function queryDatas(page){
			$(".fakeloader").show();
			$.get("${buyer_weixin_domain}/view/member/refund?page="+page+"&ts="+new Date(),
		   	function(result){
		...
		function doWxMethod() {
        	if(wx) {
        		...
        		wx.onMenuShareTimeline({
					title: '平安银行橙e券', // 分享标题
					link: '${buyer_weixin_domain}/view/weixin/refundcoupon', // 分享链接
