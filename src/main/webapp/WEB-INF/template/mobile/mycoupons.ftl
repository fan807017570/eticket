...
.thereisnomore1 p {
  ...
  border-radius: 5px;
	...
	<a class="popCE">
			领券成功！ <img src="${buyer_portal_static_resources_domain}/img/arrow-bold.png" />
	</a>
	<div class="myTicket">
		<header class="public_header">
				<ul class="tabs"> 
					<li><a href="#" class="active">未使用</a></li>
	...
	<script>
			...
			// 窗口滚动事件
	       	$(window).scroll(function(){
	       		if(($(document).height()-$(this).scrollTop()-$(this).height()) < 50) {
	       			...
	       			{
	       				var thereisnomore = $(".coupon-list-cantainer").find(".thereisnomore1").size();
	       				if(thereisnomore == 0 ){
	       					$(".coupon-list-cantainer").append("<div class='thereisnomore1'><p>到底了，没有别的了！</p></div>");
	       				}
	       			}
	       		}
			...
			function queryDatas(page){
				$(".fakeloader").show();
				$.get("${buyer_weixin_domain}/view/member/owned?page="+page+"&ts="+new Date(), 
			  	function(result){
...
// 						<#if freecoupon??>
						if(page==1)
				   			$('.popCE').slideDown('slow').delay(3000).slideUp('normal');
// 				   		</#if>
					}else{
						if(page==1) {
			...
			function doWxMethod() {
	        	if(wx) {
	        		...
	        		wx.onMenuShareTimeline({
						title: '平安银行橙e券', // 分享标题
						link: '${buyer_weixin_domain}/view/weixin/mycoupon', // 分享链接
